...
```{r}
#| code-fold: true
datos <- read_csv("./Data_input/forest_health_data_with_target.csv")
datos$Health_Status <- factor(datos$Health_Status, levels=c("Unhealthy", "Sub-healthy", "Healthy", "Very Healthy"))
...
barplot <- ggplot(datos) +
  geom_bar(aes(Health_Status, fill=Health_Status), color="black", show.legend = FALSE) +
  labs(
...
for (val_y in num_var) {
  box_graf <- ggplot(datos)+
    ...
    geom_jitter(aes(x=Health_Status, y=eval(as.name(val_y)) ), color="firebrick", alpha=0.5)+
    stat_summary(aes(x=Health_Status, y=eval(as.name(val_y))),fun.y=mean, geom="point", shape=20, size=5, color="red", fill="red") +
    labs(y=val_y)+
    theme_bw()
...
lda_plot <- ggplot(lda_data_graf, aes(x=LD1, y=LD2))+
  ...
  scale_fill_manual(values=c("Unhealthy"="darkblue","Sub-healthy"="firebrick", "Healthy" = "yellowgreen", "Very Healthy"="deeppink"))+
  labs(x= "LD1", 
       y= "LD2", fill=" ")+
    #theme_bw()+
    theme(
